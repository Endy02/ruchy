...
class NBACleaner:
    def __init__(self):
        self.ranking_data = self.__ranking_data_cleaning(
            os.path.dirname(os.path.abspath("data")) + "/data/ranking.csv")
        self.game_data = self.__game_data_cleaning(
            os.path.dirname(os.path.abspath("data")) + "/data/games.csv")
        
        
    def get_formated_data(self):
        _games = self.game_data[self.game_data['SEASON'] >= 2007]
        ...
        games_formated = games_formated.loc[games_formated["SEASON"] >= 2007].reset_index(drop=True)
        games_formated.to_csv(os.path.dirname(os.path.abspath("data")) + "/data/games_formated.csv", index=False)
    ...
    def prepare_data(self, games):
        ...
        team_info = games[['GAME_ID','HOME_TEAM_ID', 'VISITOR_TEAM_ID','PTS_home', 'FG_PCT_home', 'FT_PCT_home', 'FG3_PCT_home', 'AST_home', 'REB_home',
    ...
    def __game_data_cleaning(self, game_link):
        ...
        df = df.loc[df['GAME_DATE_EST'] >= "2007-10-28"].reset_index(drop=True)
        return df
    ...
    def __ranking_data_cleaning(self, ranking_link):
        ...
        rf = rf.loc[rf['STANDINGSDATE'] >= "2007-10-28"].reset_index(drop=True)
        return rf
